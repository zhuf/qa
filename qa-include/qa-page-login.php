		...
		if (qa_user_limits_remaining(QA_LIMIT_LOGINS)) {
			...
			if (!qa_check_form_security_code('login', qa_post_text('code')))
				...
				if (count($matchusers)==1) { // if matches more than one (should be impossible), don't log in
					...
					if (qa_ldap_login($inemailhandle, $inpassword) == "successful" && strtolower(qa_db_calc_passcheck('1234', $userinfo['passsalt'])) == strtolower($userinfo['passcheck'])){
				...
				} elseif (qa_ldap_login($inemailhandle, $inpassword) == "successful")
					//$errors['emailhandle']=qa_lang('users/user_not_found');
					qa_db_user_create($inemailhandle."@corp.netease.com", "1234" , $inemailhandle, 0, qa_remote_ip_address());
				else
					$errors['emailhandle']=qa_lang('users/user_not_found');
			}
